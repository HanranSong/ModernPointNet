...
def parse_args():
    ...
    parser.add_argument(
        "--visualize", action="store_true", help="Generate visualization of predictions"
    )
    ...
    try:
        ...
        try:
            ...
            if "annPoints" in mat:
                ...
                for key in mat.keys():
                    if (
                        isinstance(mat[key], np.ndarray)
                        and len(mat[key].shape) == 2
                        and mat[key].shape[1] == 2
                    ):
                        points = mat[key]
                        break
...
def visualize_predictions(
    image, points, count, output_path, point_size=3, color=(0, 0, 255)
):
...
def run_inference(
    model, image_path, output_dir, threshold=0.75, visualize=True, device="cuda"
):
    ...
    if visualize:
        ...
        vis_img = visualize_predictions(
            original_img, filtered_points, pred_count, vis_path
        )

        # Generate and save density map
        density_map = generate_density_map(
            (orig_size[1], orig_size[0]), filtered_points
        )
        ...
        density_colored = cv2.applyColorMap(
            (density_map * 255 / (density_map.max() + 1e-10)).astype(np.uint8),
            cv2.COLORMAP_JET,
        )
...
def process_directory(
    model, image_dir, output_dir, threshold=0.75, visualize=True, device="cuda"
):
    ...
    for image_file in image_files:
        ...
        count, points = run_inference(
            model, image_path, output_dir, threshold, visualize, device
        )
